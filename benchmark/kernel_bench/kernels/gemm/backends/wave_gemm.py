...
class WaveGemmBenchmark(WaveKernelBenchmark):
    ...
    def setup_parameters(self):
        ...
        self.BLOCK_M = self.add_param(
            "BLOCK_M",
            IntegerBounds(min=16, max=min(256, self.config.M), step=16),
            initial_value=64,
            clamp_value=True,
        )
        self.BLOCK_N = self.add_param(
            "BLOCK_N",
            IntegerBounds(min=16, max=min(256, self.config.N), step=16),
            initial_value=32,
            clamp_value=True,
        )
        self.BLOCK_K = self.add_param(
            "BLOCK_K",
            IntegerBounds(min=16, max=min(256, self.config.K), step=16),
            initial_value=64,
            clamp_value=True,
        )
    ...
    def extra_compile_options(self):
        ...
        if self.config.dtype == "bf16":
            ...
            options.postprocess = get_unroll_pipeline(self.UNROLL_FACTOR.value)
